...
namespace cAlgo
{
    ...
    public class MtfFvgX2MkCt : Indicator
    {
        ...
        // FIX 1: nomiticolor = color.new(color.yellow, 85)
        // alpha = 255 * (1 - 0.85) ≈ 38
        private static readonly Color NomitiColor      = Color.FromArgb(38, 255, 255, 0);
        ...
        {
            var total = MaxChart + Max5m + Max10m + Max15m + Max30m + Max1h
                      + Max4h + Max8h + Max12h + MaxDaily + MaxWeekly + MaxMonthly;
            if (total > 500)
        ...
        private void AddFvg(string tfKey, bool bull, double top, double bottom)
        {
            ...
            var rect = Chart.DrawRectangle(id, ShiftFromCurrentBar(5), top,
                                               ShiftFromCurrentBar(15), bottom, c);
            rect.IsFilled      = true;
            ...
            if (ShowLabels)
                label = Chart.DrawText(id + "_lbl", tfKey,
                                       ShiftFromCurrentBar(LabelShift),
                                       (top + bottom) / 2.0, LabelColor);

            (bull ? _bullByTf[tfKey] : _bearByTf[tfKey]).Add(new FvgZone
            {
                Id             = id,
                IsBull         = bull,
                Top            = top,
        ...
        private void UpdateExistingFvgs(List<FvgZone> zones, bool bull, string tfKey, int idx)
        {
            ...
            for (var i = zones.Count - 1; i >= 0; i--)
            {
                ...
                if ((mode == MitigationMode.Normal || mode == MitigationMode.None)
                    && intrusion && IncursionAlerts)
                    Print("{0} FVG Incursion {1}", bull ? "Bull" : "Bear", tfKey);

                if (EntryChangeColor)
                {
                    var entered = bull ? Bars.LowPrices[idx] < z.Top
                                       : Bars.HighPrices[idx] > z.Bottom;
                    z.Rect.Color = entered
                        ? (bull ? EntryBullColor : EntryBearColor)
                        : (bull ? BullFvgColor   : BearFvgColor);
                }
                ...
                // FIX 1: MitigationMode.None — color zone mitigated-yellow when filled
                // Pine: set_bgcolor(nomiticolor) when price crosses the far boundary.
                // Zone is NOT deleted; it stays permanently with the mitigated color.
                if (mode == MitigationMode.None)
                {
                    var penetrated = bull
                        ? (UseBodyMitigation ? Bars.ClosePrices[idx] < z.Bottom
                                             : Bars.LowPrices[idx]   < z.Bottom)
                ...
                z.Rect.Y1    = z.Top;
                z.Rect.Y2    = z.Bottom;
                z.Rect.Time1 = ShiftFromCurrentBar(5);
                z.Rect.Time2 = ShiftFromCurrentBar(15);

                if (ShowLabels && z.Label != null)
                {
                    z.Label.Time = ShiftFromCurrentBar(5);
                    z.Label.Y    = (z.Top + z.Bottom) / 2.0;
                }
            }
        }
        ...
        private void ProcessOverlaySubsystem(int idx)
        {
            ...
            // FIX 2: ATR uses Wilder's RMA matching Pine's ta.atr(length)
        ...
        private string GetOverlayBoxText()
        {
            ...
            switch (BoxType)
            {
                case OverlayBoxType.Gap:  return "GAP • "  + tfLabel;
                case OverlayBoxType.Wick: return "WICK • " + tfLabel;
                default:                  return tfLabel;
            }
        }

        private void AddOverlay(DateTime left, double top, DateTime right, double bottom,
                                Color fillColor, bool isUp)
        {
            ...
            var txt = Chart.DrawText(id + "_txt", GetOverlayBoxText(),
                                     right, (top + bottom) / 2.0, LabelColor);
            txt.IsInteractive = false;

            var tLine = ShowTopLine    ? Chart.DrawTrendLine(id + "_t", left, top,                right, top,                OverlayLineColor) : null;
            var mLine = ShowMiddleLine ? Chart.DrawTrendLine(id + "_m", left, (top + bottom)/2.0, right, (top + bottom)/2.0, OverlayLineColor) : null;
            var bLine = ShowBottomLine ? Chart.DrawTrendLine(id + "_b", left, bottom,             right, bottom,             OverlayLineColor) : null;

            if (tLine != null) tLine.LineStyle = LineStyle.Dots;
            if (mLine != null) mLine.LineStyle = LineStyle.Dots;
            if (bLine != null) bLine.LineStyle = LineStyle.Dots;
        ...
        // Verbatim from original GitHub CS — single 'right' variable unchanged
        private void UpdateOverlayLifecycle(int idx)
        {
            ...
            for (var i = _overlayZones.Count - 1; i >= 0; i--)
            {
                ...
                {
                    // Full Fill: Pine script's current implementation effectively
                    // checks the LOWER boundary (box.get_bottom) for all box types
                    filled = Cross(lower);
                }
        ...
        {
            if (string.IsNullOrEmpty(_chartTfKey)) return false;
        ...
        // ─────────────────────────────────────────────────────────────────────
        //  ATR — FIX 2: Wilder's RMA matching Pine's ta.atr(length)
        //
        //  Original used SMA(TR) which diverges from Pine by 5-15%.
        //  Wilder's RMA: rma[i] = (rma[i-1]*(length-1) + TR[i]) / length
        //  Warmup = 4× length to ensure convergence.
        //  Only affects ConditionType = ATR (default = None → no impact).
        // ─────────────────────────────────────────────────────────────────────

        private double GetAtrOnTf(Bars tfBars, int tfIndex, int length)
        {
            if (length <= 0 || tfIndex < 1) return 0.0;
            var start = Math.Max(1, tfIndex - length * 4);
            var atr   = 0.0;
            var init  = false;
            for (var j = start; j <= tfIndex; j++)
            {
                var tr = GetTrueRange(tfBars, j);
        ...
        {
            if (i <= 0) return bars.HighPrices[i] - bars.LowPrices[i];
        ...
        private DateTime ShiftFromCurrentBar(int barsToRight)
        {
            ...
            if (Bars.Count < 2) return last.AddMinutes(barsToRight);
