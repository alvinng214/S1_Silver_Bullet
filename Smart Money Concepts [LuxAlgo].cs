    ...
    public class SmartMoneyConceptsLuxAlgo : Indicator
    {
        ...
        private void ManageObList(List<OrderBlock> list, int index, bool bullish, int keep, bool show, Color color, double threshold)
        {
            for (var i = list.Count - 1; i >= 0; i--)
            {
                ...
                // invalidation/removal only on close beyond OB boundary (same spirit as Lux / reference behavior)
                var broken = bullish ? Bars.ClosePrices[index] < ob.Bottom : Bars.ClosePrices[index] > ob.Top;
                if (broken)
                {
                    DeleteObVisual(ob);
                    list.RemoveAt(i);
                ...
                // mitigation toggles style; does not remove the OB
                var mitigationTouch = OrderBlockMitigationInput == MitigationMode.Close
                    ? (bullish ? Bars.ClosePrices[index] <= ob.Top : Bars.ClosePrices[index] >= ob.Bottom)
                    : (bullish ? Bars.LowPrices[index] <= ob.Top : Bars.HighPrices[index] >= ob.Bottom);
                ...
                var baseColor = ob.Mitigated
                    ? Color.FromArgb(90, color.R, color.G, color.B)
                    : color;
                var style = ob.Mitigated ? LineStyle.DotsRare : LineStyle.Solid;

                if (ob.Box == null)
                {
                    var rect = Chart.DrawRectangle(ob.Id, ob.Index, ob.Top, right, ob.Bottom, baseColor, 1, style);
                    rect.IsFilled = true;
