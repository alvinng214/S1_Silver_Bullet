...
namespace cAlgo
{
    ...
    public class IctSetup01FvgLiquidityHuntIndicator : Indicator
    {
        ...
        private bool _pendingLongSignalOnClose;
        private bool _pendingShortSignalOnClose;
        ...
        private void DrawBullDiscountPremium(int point)
        {
            ...
            Chart.DrawTrendLine(discountLineId, point + 12, _bullishDiscount, point + 20, _bullishDiscount, Color.Transparent);
            Chart.DrawTrendLine(premiumLineId, point + 12, _bullishPremium, point + 20, _bullishPremium, Color.Transparent);
            Chart.DrawTrendLine(equilibriumLineId, point + 12, _bullishEquilibrium, point + 20, _bullishEquilibrium, Color.Transparent);

            var disRect = Chart.DrawRectangle(disRectId, point + 12, _bullishDiscount, point + 20, _bullishEquilibrium, Color.FromArgb(110, 211, 238, 255));
            disRect.IsFilled = true;
            disRect.IsInteractive = false;

            var premRect = Chart.DrawRectangle(premRectId, point + 12, _bullishPremium, point + 20, _bullishEquilibrium, Color.FromArgb(75, 255, 165, 100));
            premRect.IsFilled = true;
            premRect.IsInteractive = false;

            Chart.DrawText(disLabelId, "Discount", point + 16, _bullishDiscount, Color.Black);
            Chart.DrawText(premLabelId, "Premium", point + 16, _bullishPremium, Color.Black);
            Chart.DrawText(equiLabelId, "EQU", point + 16, _bullishEquilibrium, Color.Black);
        }
        ...
        private void DrawBearDiscountPremium(int point)
        {
            ...
            Chart.DrawTrendLine(premiumLineId, point + 12, _bearishPremium, point + 20, _bearishPremium, Color.Transparent);
            Chart.DrawTrendLine(discountLineId, point + 12, _bearishDiscount, point + 20, _bearishDiscount, Color.Transparent);
            Chart.DrawTrendLine(equilibriumLineId, point + 12, _bearishEquilibrium, point + 20, _bearishEquilibrium, Color.Transparent);

            var disRect = Chart.DrawRectangle(disRectId, point + 12, _bearishDiscount, point + 20, _bearishEquilibrium, Color.FromArgb(110, 211, 238, 255));
            disRect.IsFilled = true;
            disRect.IsInteractive = false;

            var premRect = Chart.DrawRectangle(premRectId, point + 12, _bearishPremium, point + 20, _bearishEquilibrium, Color.FromArgb(75, 255, 165, 100));
            premRect.IsFilled = true;
            premRect.IsInteractive = false;

            Chart.DrawText(disLabelId, "Discount", point + 16, _bearishDiscount, Color.Black);
            Chart.DrawText(premLabelId, "Premium", point + 16, _bearishPremium, Color.Black);
            Chart.DrawText(equiLabelId, "EQU", point + 16, _bearishEquilibrium, Color.Black);
        }
        ...
        private void EmitAlerts(int index)
        {
            ...
            if (string.Equals(frequencyMode, "Per Bar Close", StringComparison.Ordinal))
            {
                if (isNewLiveBar)
                {
                    var closedBarIndex = index - 1;
                    if (_pendingLongSignalOnClose && _lastLongAlertBar != closedBarIndex)
                    {
                        Print("{0} | LONG | Bar={1} | TZ={2} | {3}", AlertName, closedBarIndex, AlertTimeZone, LongPositionMessage);
                        _lastLongAlertBar = closedBarIndex;
                    }

                    if (_pendingShortSignalOnClose && _lastShortAlertBar != closedBarIndex)
                    {
                        Print("{0} | SHORT | Bar={1} | TZ={2} | {3}", AlertName, closedBarIndex, AlertTimeZone, ShortPositionMessage);
                        _lastShortAlertBar = closedBarIndex;
                    }

                    _pendingLongSignalOnClose = false;
                    _pendingShortSignalOnClose = false;
                }

                _pendingLongSignalOnClose = _pendingLongSignalOnClose || _isLongSignal;
                _pendingShortSignalOnClose = _pendingShortSignalOnClose || _isShortSignal;
            ...
            if (_isLongSignal)
            {
                if (string.Equals(frequencyMode, "All", StringComparison.Ordinal) || _lastLongAlertBar != index)
                {
                    Print("{0} | LONG | Bar={1} | TZ={2} | {3}", AlertName, index, AlertTimeZone, LongPositionMessage);
                    if (!string.Equals(frequencyMode, "All", StringComparison.Ordinal))
            ...
            if (_isShortSignal)
            {
                if (string.Equals(frequencyMode, "All", StringComparison.Ordinal) || _lastShortAlertBar != index)
                {
