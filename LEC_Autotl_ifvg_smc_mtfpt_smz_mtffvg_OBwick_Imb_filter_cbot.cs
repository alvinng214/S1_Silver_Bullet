...
namespace cAlgo
{
    ...
    public class LEC_Autotl_ifvg_smc_mtfpt_smz_mtffvg_OBwick_Imb_filter_cbot : Robot
    {
        // ENUMS
        ...
        [Parameter("Enable H1 LEC",          DefaultValue = true,  Group = "LEC Settings")] public bool LecEnableH1      { get; set; }
        [Parameter("Enable H4 LEC",          DefaultValue = false, Group = "LEC Settings")] public bool LecEnableH4      { get; set; }
        [Parameter("Enable Current TF LEC",  DefaultValue = false, Group = "LEC Settings")] public bool LecEnableCurrent { get; set; }
        [Parameter("Apply Stop Hunt Wick Filter", DefaultValue = true, Group = "LEC Settings")] public bool LecFilterLiquidity { get; set; }
        [Parameter("Apply Close Filter",     DefaultValue = true,  Group = "LEC Settings")] public bool LecFilterClose   { get; set; }

        // ═══ PARAMETERS — Displacement Settings ═══════════════════════════════

        [Parameter("Require FVG",              DefaultValue = true,                              Group = "Displacement Settings")] public bool            DispRequireFvg { get; set; }
        [Parameter("Displacement Type",        DefaultValue = DisplacementType.OpenToClose,      Group = "Displacement Settings")] public DisplacementType DispType       { get; set; }
        [Parameter("Displacement Length",      DefaultValue = 100, MinValue = 1,                 Group = "Displacement Settings")] public int             DispStdLen     { get; set; }
        [Parameter("Displacement Strength",    DefaultValue = 2,   MinValue = 0,                 Group = "Displacement Settings")] public int             DispStdX       { get; set; }
        ...
        protected override void OnStart()
        {
            ...
            if (LecEnableH1 && (EnableLecLong || EnableLecShort))
                _lecH1Bars = MarketData.GetBars(TimeFrame.Hour);
            if (LecEnableH4 && (EnableLecLong || EnableLecShort))
                _lecH4Bars = MarketData.GetBars(TimeFrame.Hour4);
        ...
        protected override void OnStop() => Print("LEC_Autotl_ifvg_smc_mtfpt_smz_mtffvg_OBwick_Imb_filter_cbot stopped.");

        // ═══ LEC ENGINE ═══════════════════════════════════════════════════════

        private void DetectLecSignal(int bar, out bool bullOut, out bool bearOut)
        {
            bullOut = false; bearOut = false;
            if (bar < 1) return;
            if (LecEnableH1 && _lecH1Bars != null) CheckLecOnTf(_lecH1Bars, bar, ref bullOut, ref bearOut, ref _lecLastH1Idx);
            if (LecEnableH4 && _lecH4Bars != null) CheckLecOnTf(_lecH4Bars, bar, ref bullOut, ref bearOut, ref _lecLastH4Idx);
            if (LecEnableCurrent)                   CheckLecOnTf(Bars,       bar, ref bullOut, ref bearOut, ref _lecLastCurIdx);
        }

        private void CheckLecOnTf(Bars src, int bar, ref bool bull, ref bool bear, ref int lastIdx)
        {
        ...
        private void EvalLec(Bars b, int i, out bool bullEng, out bool bearEng)
        {
            bullEng = false; bearEng = false;
            if (b == null || i < 1 || i >= b.Count) return;
            double po=b.OpenPrices[i-1],pc=b.ClosePrices[i-1],co=b.OpenPrices[i],cc=b.ClosePrices[i];
            bullEng = co<=pc && co<po && cc>po;
            bearEng = co>=pc && co>po && cc<po;
            if (LecFilterLiquidity) { bullEng=bullEng&&b.LowPrices[i]<=b.LowPrices[i-1]; bearEng=bearEng&&b.HighPrices[i]>=b.HighPrices[i-1]; }
            if (LecFilterClose)     { bullEng=bullEng&&b.ClosePrices[i]>=b.HighPrices[i-1]; bearEng=bearEng&&b.ClosePrices[i]<=b.LowPrices[i-1]; }
        }

        // ═══ DISPLACEMENT ENGINE ══════════════════════════════════════════════

        private void DetectDisplacementSignal(int bar, out bool bullOut, out bool bearOut)
        {
            bullOut = false; bearOut = false;
            bool isDis; int dispBar;
            if (DispRequireFvg)
            {
                if (bar < 2) return;
                isDis = IsDispWithFvg(bar);
                dispBar = bar - 1;
            }
            else
            {
                isDis = IsDispNoFvg(bar);
                dispBar = bar;
            }
            if (!isDis) return;
            bool isBull = Bars.ClosePrices[dispBar] > Bars.OpenPrices[dispBar];
            bullOut = isBull; bearOut = !isBull;
        }

        private bool IsDispWithFvg(int i)
        {
            if (i < 2) return false;
            double range = DispRange(i-1);
            double std   = DispStd(i-1);
            if (double.IsNaN(std) || std <= 0) return false;
            bool fvg = Bars.ClosePrices[i-1] > Bars.OpenPrices[i-1]
                ? Bars.HighPrices[i-2] < Bars.LowPrices[i]
                : Bars.LowPrices[i-2]  > Bars.HighPrices[i];
            return range > std && fvg;
        }

        private bool IsDispNoFvg(int i)
        {
            double std = DispStd(i);
            return !double.IsNaN(std) && std > 0 && DispRange(i) > std;
        }

        private double DispRange(int i)
        {
            if (i < 0 || i >= Bars.Count) return double.NaN;
            return DispType == DisplacementType.OpenToClose
                ? Math.Abs(Bars.OpenPrices[i] - Bars.ClosePrices[i])
                : Bars.HighPrices[i] - Bars.LowPrices[i];
        }

        private double DispStd(int i)
        {
            if (i < 0 || DispStdLen <= 0) return double.NaN;
            int s = i - DispStdLen + 1;
            if (s < 0) return double.NaN;
            double sum = 0;
            for (int k = s; k <= i; k++) sum += DispRange(k);
            double mean = sum / DispStdLen;
            double vsum = 0;
            for (int k = s; k <= i; k++) { double d = DispRange(k) - mean; vsum += d*d; }
            return Math.Sqrt(vsum / DispStdLen) * DispStdX;
        }
        ...
        private static string MtfTfOptStr(MtfTfOption opt)
        {
            switch(opt)
            {
