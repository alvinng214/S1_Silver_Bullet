...
//                         E (HTF mini-candle display — last 4 HTF candles to right of last bar),
//                         F (sweep marker lines on mini candles),
//                         I (HTF bias plot), K (Key levels for 3 TFs).
// Excluded by user: cross-asset screener, HTF Swept screener table, SMT divergence,
...
namespace cAlgo
{
    ...
    public class HTFCdSweepCisdCx : Indicator
    {
        // ───────────────────────────── HTF ─────────────────────────────
        ...
        [Parameter("Show HTF Candle", Group = "HTF", DefaultValue = true)]
        public bool ShowCandle { get; set; }

        [Parameter("Mini-candle offset (bars ahead)", Group = "HTF", DefaultValue = 0)]
        ...
        [Parameter("Mini-Candles Border Color", Group = "Colors", DefaultValue = "Black")]
        public Color CanBrColor { get; set; }
        ...
        public override void Calculate(int index)
        {
            ...
            // Subsystems E + F — HTF mini-candles + sweep marker lines (last bar only)
            if (IsLastBar && ShowCandle)
                DrawHtfMiniCandles(index, hSwept, lSwept);
        ...
        // ───────────── Subsystem E + F — HTF mini-candles & sweep markers ─────────────
        // Pine renders four mini "candles" to the right of the last LTF bar showing the four
        // most-recent HTF candles (current + 3 prior). Each occupies a 3-bar slot:
        //   slot 0 (most recent):  bars dx+18 .. dx+20  (wick at dx+19)
        //   slot 1:                bars dx+15 .. dx+17  (wick at dx+16)
        //   slot 2:                bars dx+12 .. dx+14  (wick at dx+13)
        //   slot 3 (oldest):       bars dx+9  .. dx+11  (wick at dx+10)
        // Subsystem F draws short horizontal lines anchored to slot-1 (the previous HTF candle's
        // h1/l1) extending toward slot-0 to highlight when the current HTF candle has swept those
        // levels (h_swept/l_swept).
        private void DrawHtfMiniCandles(int i, bool hSwept, bool lSwept)
        {
            DrawMiniSlot(i, "0", _o0, _h0, _l0, _c0, 18);
            DrawMiniSlot(i, "1", _o1, _h1, _l1, _c1, 15);
            DrawMiniSlot(i, "2", _o2, _h2, _l2, _c2, 12);
            DrawMiniSlot(i, "3", _o3, _h3, _l3, _c3, 9);

            // Subsystem F — sweep marker lines (slot-1 wick-edge → slot-0 wick anchor)
            // Bear sweep line: current HTF high above prior-HTF high, body still under it, h_swept true
            string swH = "cdc_sw_h";
            if (_h0 > _h1 && _h1 > Math.Max(_o0, _c0) && hSwept)
                Chart.DrawTrendLine(swH, i + Dx + 16, _h1, i + Dx + 19, _h1, BearColor, 2, LineStyle.Solid);
            else
                Chart.RemoveObject(swH);

            // Bull sweep line: current HTF low below prior-HTF low, body still above it, l_swept true
            string swL = "cdc_sw_l";
            if (_l0 < _l1 && _l1 < Math.Min(_o0, _c0) && lSwept)
                Chart.DrawTrendLine(swL, i + Dx + 16, _l1, i + Dx + 19, _l1, BullColor, 2, LineStyle.Solid);
            else
                Chart.RemoveObject(swL);
        }

        private void DrawMiniSlot(int i, string slotTag, double op, double hi, double lo, double cl, int leftOffset)
        {
            // Body: spans bars [i+dx+leftOffset .. i+dx+leftOffset+2], from open to close
            int bodyL = i + Dx + leftOffset;
            int bodyR = i + Dx + leftOffset + 2;
            int wickX = i + Dx + leftOffset + 1;

            Color bodyClr = cl > op ? BullColor : cl < op ? BearColor : RangeColor;

            string bodyName = "cdc_mini_body_" + slotTag;
            string wickUpName = "cdc_mini_wup_" + slotTag;
            string wickDnName = "cdc_mini_wdn_" + slotTag;
            string labelName = "cdc_mini_lbl_" + slotTag;

            // Body rectangle (filled)
            var body = Chart.DrawRectangle(bodyName, bodyL, op, bodyR, cl, bodyClr);
            body.IsFilled = true;
            // Border ring around body
            string bodyBorderName = "cdc_mini_bbr_" + slotTag;
            var bodyBorder = Chart.DrawRectangle(bodyBorderName, bodyL, op, bodyR, cl, CanBrColor, 1, LineStyle.Solid);
            bodyBorder.IsFilled = false;

            // Upper wick — vertical line from max(open,close) up to high
            Chart.DrawTrendLine(wickUpName, wickX, Math.Max(op, cl), wickX, hi, CanBrColor, 1, LineStyle.Solid);
            // Lower wick — vertical line from min(open,close) down to low
            Chart.DrawTrendLine(wickDnName, wickX, Math.Min(op, cl), wickX, lo, CanBrColor, 1, LineStyle.Solid);

            // Centred label showing HTF tag (e.g., "D", "W", "1h")
            var lbl = Chart.DrawText(labelName, F_Text(Htf), wickX, (op + cl) / 2.0, Color.White);
        ...
        private class MiniHtfState
        {
            ...
            public void Update(int i, TimeFrame htf, HTFCdSweepCisdCx ind)
            {
