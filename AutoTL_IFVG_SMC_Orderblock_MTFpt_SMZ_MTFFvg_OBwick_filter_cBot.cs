// =============================================================================
// AutoTL_IFVG_SMC_Orderblock_MTFpt_SMZ_MTFFvg_OBwick_filter_cBot
// =============================================================================
// Base    : AutoTL_IFVG_SMC_Orderblock_MTFpt_SMZ_MTFFvg_filter_cBot
// Added   : OB Wick Filter — embeds OB detection from
//           ICT MTF Order Block Wicks [MK].
//
// OB Wick Filter logic:
//   Long  allowed when >= 1 active Bull OB on any enabled TF was touched
//   (Low <= zone.Top) within the last ObWickLookbackBars chart bars.
//   Short allowed when >= 1 active Bear OB on any enabled TF was touched
//   (High >= zone.Bottom) within the last ObWickLookbackBars chart bars.
//
//   Bull OB zone: Top = prevBarHigh, Bottom = prevBarOpen (bearish candle)
//   Bear OB zone: Top = prevBarOpen, Bottom = prevBarLow  (bullish candle)
//
//   Detection method Body: requires prior bar opposite colour + current bar
//   closes through prior bar's high/low.
//   Detection method Wicks: current bar opens inside prior bar's range.
//
//   Enabled TFs: 5m, 15m, 30m, 1h, 4h, Daily.
//   All prior filter logic unchanged.
// =============================================================================
...
namespace cAlgo
{
    ...
    public class AutoTL_IFVG_SMC_Orderblock_MTFpt_SMZ_MTFFvg_OBwick_filter_cBot : Robot
    {
        // ═════════════════════════════════════════════════════════════════════
        //  ENUMS
        // ═════════════════════════════════════════════════════════════════════
        ...
        private sealed class MtfTfState
        {
            ...
            // true  = most recent structural event was a BOS  (continuation)
            // false = most recent structural event was a CHoCH (reversal)
            public bool     LastEventWasBos  = false;
            // Per-TF event-type gates — stored here so the filter is self-contained
        ...
        // ═════════════════════════════════════════════════════════════════════
        //  INNER TYPES — OB Wick Filter (no chart objects)
        // ═════════════════════════════════════════════════════════════════════
        ...
        private sealed class ObWickZone
        {
            ...
            public double Top;
            public double Bottom;
            public bool   IsMitigated; // None mode only: fully penetrated but kept alive
        }
        ...
        private sealed class ObWickTfState
        {
            ...
            public int                LastDetectedSrcIdx = -1; // last currClosedIdx detection ran on
        }

        // ═════════════════════════════════════════════════════════════════════
        //  PARAMETERS — Signal Engine Enables
        //
        //  Master switches for each signal engine and direction.
        //  A trade fires when at least one enabled engine emits in that direction.
        //  All downstream filters always apply regardless of which engine fired.
        // ═════════════════════════════════════════════════════════════════════
        ...
        //
        //  Verbatim from IFVG_SMC_Orderblock_MTFpt_SMZ_filter_cBot.
        //  Signal fires when an unbroken FVG gap is broken out through by
        //  a closing bar whose MA is trending in the same direction.
        // ═════════════════════════════════════════════════════════════════════
        ...
        [Parameter("Instance Name",             DefaultValue = "AutoTL_IFVG_SMC_MTFpt_SMZ_FVG_OBwick_cBot", Group = "Risk Management")]
        ...
        //
        //  Embeds OB detection from ICT MTF Order Block Wicks [MK] (display stripped).
        //  Long  allowed when a Bull OB on any enabled TF was touched
        //  (Low <= zone.Top) within the last ObWickLookbackBars chart bars.
        //  Short allowed when a Bear OB on any enabled TF was touched
        //  (High >= zone.Bottom) within the last ObWickLookbackBars chart bars.
        // ═════════════════════════════════════════════════════════════════════
        ...
        protected override void OnStart()
        {
            ...
            // IFVG signal engine MA — always initialised; engine is gated by
            // EnableIfvgLong / EnableIfvgShort at signal-detection time.
            ...
            if (EnableObWickFilter)
            {
                ...
                Print("OB Wick filter ON. Lookback={0} Detection={1} Mitigation={2}/{3}. " +
                      "TFs: 5m={4} 15m={5} 30m={6} 1h={7} 4h={8} D={9}",
                    ObWickLookbackBars, ObWickDetectionMethodInput,
                    ObWickMitigationModeInput, ObWickMitigationTypeInput,
                    ObWickEnable5m, ObWickEnable15m, ObWickEnable30m,
                    ObWickEnable1h, ObWickEnable4h, ObWickEnableDaily);
            }

            Print("AutoTL_IFVG_SMC_Orderblock_MTFpt_SMZ_MTFFvg_OBwick_filter_cBot started. " +
                  "PP={0}, PivotL={1}, PivotR={2}, MaxPos={3}, Risk={4}%, RR={5} | " +
                  "ATL Long={6} Short={7} | IFVG Long={8} Short={9} MA={10}/{11}",
                  PP, PivotLeft, PivotRight, MaxOpenPositions, RiskPercent, RiskRewardRatio,
                  EnableAtlLong, EnableAtlShort, EnableIfvgLong, EnableIfvgShort,
                  IfvgMaType, IfvgMaPeriod);
        }

        protected override void OnStop()
        {
            Print("AutoTL_IFVG_SMC_Orderblock_MTFpt_SMZ_MTFFvg_OBwick_filter_cBot stopped.");
        }
        ...
        protected override void OnBar()
        {
            ...
            // ── ATL signals (already set by RunAtl in the loop above) ────────
            bool atlLong  = EnableAtlLong  && _atlIsLongSignal;
            bool atlShort = EnableAtlShort && _atlIsShortSignal;

            // ── IFVG signal (detected once per bar, not incremental) ─────────
            bool ifvgLong  = false;
        ...
        // ═════════════════════════════════════════════════════════════════════
        //  IFVG SIGNAL ENGINE
        //  Verbatim port of CalculateMa / DetectIfvgSignal / DetectFvg /
        //  TryProcessFvgCandidate from IFVG_SMC_Orderblock_MTFpt_SMZ_filter_cBot.
        //  Field names prefixed _ifvg to avoid collision with the MTF FVG filter.
        // ═════════════════════════════════════════════════════════════════════
        ...
        private int IfvgDetectFvg(int currentIndex, int idx, double epsVal)
        {
            ...
            if (lt > h2 - epsVal) return  1;   // bullish gap
            if (ht < l2 + epsVal) return -1;   // bearish gap
            return 0;
        }

        private bool IfvgTryProcessCandidate(int index, int i, int fvgType,
            double minSizeValue, double maValue, out int signalDir)
        {
            signalDir = 0;
            ...
            // Gap must be unbroken between formation and current bar
            if (i > 1)
            {
                for (var k = i - 1; k >= 1; k--)
                {
        ...
        private MtfTfState MtfCreateState(string tfInput, int pivotStrength, bool isLowerTf,
            bool allowBullChoch, bool allowBullBos, bool allowBearChoch, bool allowBearBos)
        {
        ...
        private void MtfProcessCalcBar(MtfTfState s, int tfBarIndex)
        {
            ...
            if (!double.IsNaN(s.LastPivotHigh) && !double.IsNaN(prevLastPivotHigh))
                ...
                {
                    // BOS  = was already bullish AND this breaks a new (non-duplicate) high.
                    // CHoCH= was bearish and trend just flipped bullish.
                    // Mirrors breakOfStructure logic in Market Structure MTF Trend [Pt].cs.
            ...
            if (!double.IsNaN(s.LastPivotLow) && !double.IsNaN(prevLastPivotLow))
                ...
                {
                    // BOS  = was already bearish AND this breaks a new (non-duplicate) low.
                    // CHoCH= was bullish and trend just flipped bearish.
        ...
        private bool CheckMtfFilter(bool isLong, int signalBar)
        {
            ...
            if (!result)
                Print("[MTF BLOCKED] {0} bar={1} passing={2}/{3} logic={4} | TF1:{5}({6}) TF2:{7}({8}) TF3:{9}({10}) TF4:{11}({12})",
                    ...
                    MtfStateLabel(_mtfState1), _mtfState1!=null?_mtfState1.LastProcessedTfBar:-1,
                    MtfStateLabel(_mtfState2), _mtfState2!=null?_mtfState2.LastProcessedTfBar:-1,
                    MtfStateLabel(_mtfState3), _mtfState3!=null?_mtfState3.LastProcessedTfBar:-1,
                    MtfStateLabel(_mtfState4), _mtfState4!=null?_mtfState4.LastProcessedTfBar:-1);
            return result;
        }

        /// <summary>
        /// Returns true when this TF state counts as a passing vote for isLong.
        ///   Warmup (HasTrend=false)   → always pass through.
        ///   Wrong direction           → always block.
        ///   Correct direction         → check this TF's own event-type toggles.
        ///   Both toggles OFF for this direction → failsafe: treat as both ON.
        /// </summary>
        private static bool MtfStatePassesTypeFilter(MtfTfState s, bool isLong)
        {
            if (!s.HasTrend)              return true;   // warmup
            if (s.CurrentTrend != isLong) return false;  // wrong direction

            bool allowChoch = isLong ? s.AllowBullChoch : s.AllowBearChoch;
            bool allowBos   = isLong ? s.AllowBullBos   : s.AllowBearBos;

            if (!allowChoch && !allowBos) return true;   // both off → no restriction

            return s.LastEventWasBos ? allowBos : allowChoch;
        }

        private static string MtfStateLabel(MtfTfState s)
        {
            if (s == null)       return "Off";
            if (!s.HasTrend)     return "Warm";
            return (s.CurrentTrend ? "Bull" : "Bear") +
                   (s.LastEventWasBos ? "(BOS)" : "(CHoCH)");
        }
        ...
        //  Ported from ICT MTF Order Block Wicks [MK] indicator.
        ...
        private void ObWickRegisterTf(string key, TimeFrame tf, bool enabled, int max, string label)
        {
            if (!enabled) return;
            var bars = tf == Bars.TimeFrame ? Bars : MarketData.GetBars(tf);
            _obWickStates.Add(new ObWickTfState
            {
                TfBars   = bars,
                Label    = label,
                MaxCount = max
        ...
        /// <summary>
        /// Called once per chart bar in the incremental loop.
        /// Detects new OB zones from newly closed HTF bars and
        /// runs per-bar mitigation on all existing zones.
        /// </summary>
        private void RunObWickFilter(int chartIndex)
        {
            ...
            foreach (var state in _obWickStates)
            {
                ...
                // Find the most recent HTF bar that has fully opened before chartTime
                var srcIdx = MtfFindAtOrBefore(tfBars, chartTime);
                if (srcIdx < 3) continue;

                // currClosedIdx = last CLOSED HTF bar (mirrors indicator's srcIdx-1)
                var currClosedIdx = srcIdx - 1;
                var prevIdx       = currClosedIdx - 1;

                if (prevIdx < 0) continue;

                // ── Detection: only runs once per new HTF bar ─────────────────
                if (inSession && currClosedIdx > state.LastDetectedSrcIdx)
                {
                    ...
                    // Bull OB detection — mirrors IsBullDetected in indicator
                    bool isBull = ObWickDetectionMethodInput == ObWickDetectionMethod.Body
                        ? open1 > close1 && op < cl && cl > high1   // body: prev bear, curr bull closes above prev high
                        : op < high1;                                // wicks: curr opens inside prev bar's range

                    // Bear OB detection — mirrors IsBearDetected in indicator
                    bool isBear = ObWickDetectionMethodInput == ObWickDetectionMethod.Body
                        ? open1 < close1 && op > cl && cl < low1    // body: prev bull, curr bear closes below prev low
                        : op < low1;                                 // wicks: curr opens below prev bar's low

                    if (isBull)
                    {
                        // Bull OB zone: Top = prevBarHigh, Bottom = prevBarOpen
                        double zTop    = Math.Max(high1, open1);
                        double zBottom = Math.Min(high1, open1);
                        if (!ObWickHasDuplicateTop(state.Bulls, zTop))
                        {
                            if (state.Bulls.Count >= state.MaxCount)
                                state.Bulls.RemoveAt(0);
        ...
        private void ObWickMitigateBullZones(ObWickTfState state, double low, double close)
        {
            ...
            for (int k = state.Bulls.Count - 1; k >= 0; k--)
            {
                ...
                {
                    // Shrink top boundary toward price
        ...
        private void ObWickMitigateBearZones(ObWickTfState state, double high, double close)
        {
            ...
            for (int k = state.Bears.Count - 1; k >= 0; k--)
            {
                ...
                if (mode == ObWickMitigationMode.Dynamic)
                {
                    // Shrink bottom boundary toward price
        ...
        /// <summary>
        /// OB Wick filter gate. Returns true when the filter is off, no TFs
        /// are registered, or the touch vote is satisfied.
        ///
        /// Touch definition (matches the indicator's wick-incursion concept):
        ///   Bull OB: Bars.Low[b]  &lt;= zone.Top    (wick enters zone from above)
        ///   Bear OB: Bars.High[b] &gt;= zone.Bottom (wick enters zone from below)
        /// </summary>
        private bool CheckObWickFilter(bool isLong, int signalBar)
        {
            ...
            foreach (var state in _obWickStates)
            {
                ...
                foreach (var z in zones)
                {
                    ...
                    {
                        bool touched = isLong
                            ? Bars.LowPrices[b]  <= z.Top       // low wicks into bull OB
                            : Bars.HighPrices[b] >= z.Bottom;   // high wicks into bear OB
                        if (touched) return true;
                    }
                }
            }

            Print("[OB BLOCKED] {0} bar={1} lookback={2} — no OB wick touch found.",
                isLong ? "Long" : "Short", signalBar, ObWickLookbackBars);
            return false;
        }
