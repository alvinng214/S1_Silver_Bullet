...
namespace cAlgo
{
    ...
    public class SMC_orderblock_detector : Indicator
    {
        ...
        [Parameter("Use Chart Timeframe", Group = "Order-Block Detector", DefaultValue = true)]
        public bool UseChartTimeframe { get; set; }
        [Parameter("Time-Frame Order-Block", Group = "Order-Block Detector", DefaultValue = "Hour")]
        public TimeFrame InputTimeFrame { get; set; }
        [Parameter("Line width Liquidated", Group = "Order-Block Detector", DefaultValue = 1, MinValue = 1, MaxValue = 4)]
        public int LineWidthLiquidated { get; set; }
        [Parameter("Transparency", Group = "Order-Block Detector", DefaultValue = 80, MinValue = 1, MaxValue = 100)]
        public int Transparency { get; set; }
        [Parameter("Color Bull", Group = "Order-Block Detector", DefaultValue = "Green")]
        public Color ColorBull { get; set; }
        [Parameter("Color Bear", Group = "Order-Block Detector", DefaultValue = "Red")]
        public Color ColorBear { get; set; }
        [Parameter("Color FVG Bull", Group = "Order-Block Detector", DefaultValue = "Blue")]
        public Color ColorFvgBull { get; set; }
        [Parameter("Color FVG Bear", Group = "Order-Block Detector", DefaultValue = "Orange")]
        public Color ColorFvgBear { get; set; }
        [Parameter("Show Order-Blocks", Group = "Order-Block Detector", DefaultValue = true)]
        public bool ShowOb { get; set; }
        [Parameter("Show Fair-Value-Gaps", Group = "Order-Block Detector", DefaultValue = true)]
        public bool ShowFvg { get; set; }
        [Parameter("Show Signals Order-Block", Group = "Order-Block Detector", DefaultValue = true)]
        public bool ShowSignalsOb { get; set; }
        [Parameter("Show Signals FVG", Group = "Order-Block Detector", DefaultValue = true)]
        public bool ShowSignalsFvg { get; set; }
        [Parameter("Min dist", Group = "Order-Block Detector", DefaultValue = 1, MinValue = 1)]
        public int MinDist { get; set; }
        [Parameter("Min dist FVG", Group = "Order-Block Detector", DefaultValue = 1, MinValue = 1)]
        public int MinDistFvg { get; set; }
        [Parameter("Use Heikin-Ashi", Group = "Order-Block Detector", DefaultValue = false)]
        public bool UseHeikinAshi { get; set; }
        [Parameter("Signal Offset (pips)", Group = "Order-Block Detector", DefaultValue = 2.0, MinValue = 0.0, Step = 0.1)]
        public double SignalOffsetPips { get; set; }
        [Parameter("Signal Bull Color", Group = "Order-Block Detector", DefaultValue = "Lime")]
        public Color SignalBullColor { get; set; }
        [Parameter("Signal Bear Color", Group = "Order-Block Detector", DefaultValue = "Red")]
        public Color SignalBearColor { get; set; }
        ...
        private SignalState _obSignal = NewEmptySignal();
        ...
        public override void Calculate(int index)
        {
            ...
            if (ShowOb && (ShowInternalOrderBlocksInput || ShowSwingOrderBlocksInput))
                UpdateOrderBlocks(index);

            if (ShowFvg && ShowFairValueGapsInput)
                UpdateFvgs(index);
            ...
            EvaluateSignalEntry(index, sourceIndex);
        }
        ...
        private void ManageObList(List<OrderBlock> list, int index, bool bullish, int keep, bool show, Color color)
        {
            for (var i = list.Count - 1; i >= 0; i--)
            {
                ...
                if (crossedOrderBlock)
                {
                    ...
                    {
                        var point = bullish ? ob.Bottom : ob.Top;
                        _obSignal = NewSignal(index, point, bullish);
                        DrawLiquidationLine($"ob_line_{ob.Id}_{index}", ob.Time, Bars.OpenTimes[index], point, bullish ? ColorBull : ColorBear);
                    }
        ...
        private void ResetSignals(int index)
        {
            ...
            LongSignal[index] = ShortSignal[index] = double.NaN;
        }

        private void EvaluateSignalEntry(int index, int sourceIndex)
        {
            if (sourceIndex < 0 || sourceIndex >= _sourceBars.Count)
                return;

            var sourceClose = _sourceBars.ClosePrices[sourceIndex];
            var signalClose = UseHeikinAshi ? _haSourceClose[sourceIndex] : sourceClose;
            var candleDir = Bars.ClosePrices[index] > Bars.OpenPrices[index] ? 1 : -1;

            var cond = 0;
            if (!double.IsNaN(_obSignal.Point) && signalClose > _obSignal.Point && _obSignal.IsBull && candleDir == 1 && !_obSignal.Entry)
            {
            ...
            DrawOrderBlockSignals(index, cond);
        }

        private void DrawOrderBlockSignals(int index, int cond)
        {
            var offset = SignalOffsetPips * Symbol.PipSize;

            if (ShowSignalsOb && cond == 1)
                DrawSignalIcon($"ob_buy_{index}_{_shapeId++}", ChartIconType.UpArrow, index, Bars.LowPrices[index], SignalBullColor, -offset);
            if (ShowSignalsOb && cond == -1)
                DrawSignalIcon($"ob_sell_{index}_{_shapeId++}", ChartIconType.DownArrow, index, Bars.HighPrices[index], SignalBearColor, offset);
