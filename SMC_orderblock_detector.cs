...
namespace cAlgo
{
    ...
    public class SMC_OrderBlock_Detector : Indicator
    {
        ...
        /// <summary>
        /// Determines which price is compared against the structure level when
        /// deciding whether a BOS / CHoCH has occurred.
        ///   Close   – original behaviour: bar close must cross the level.
        ///   HighLow – more aggressive: bar high (bullish) or bar low (bearish)
        ///             must cross the level.
        /// </summary>
        ...
        /// <summary>
        /// Price source used to detect iBOS / iCHoCH crosses.
        /// Close = original behaviour.  HighLow = High/Low cross triggers the event.
        /// </summary>
        ...
        /// <summary>
        /// Number of bars on each side of a bar used to qualify it as an internal pivot.
        /// Larger values = fewer, more significant pivots.  Default 5 (original hardcoded value).
        /// </summary>
        ...
        /// <summary>
        /// Minimum number of bars that must have closed AFTER the structure-break
        /// bar (BOS / iBOS / CHoCH / iCHoCH) before an OB that was created by that
        /// break is allowed to trigger a long or short signal.
        ///
        /// 0 = no restriction (default — same as original behaviour).
        /// 1 = at least 1 bar must close after the structure break.
        ///
        /// NOTE: this counts bars after the structure event, NOT after the OB pivot.
        /// </summary>
        ...
        private sealed class OrderBlock
        {
            ...
            /// <summary>
            /// Bar index of the BOS/CHoCH/iBOS/iCHoCH event that created this OB.
            /// Used to:
            ///   (a) block a signal when the touch happens on the SAME bar as the
            ///       structure break (Filter 1).
            ///   (b) enforce the MinBarsAfterStructureBreak cooldown (Filter 2).
            /// </summary>
        ...
        private int    _lastSwingHighIndex = -1,      _lastSwingLowIndex  = -1;
        ...
        private int    _internalHighIndex = -1, _internalLowIndex = -1;

        private double _prevEqHigh = double.NaN; private int _prevEqHighIndex = -1;
        private double _prevEqLow  = double.NaN; private int _prevEqLowIndex  = -1;
        ...
        // ── Signal engine ─────────────────────────────────────────────────────
        private SignalState _internalObSignal;
        ...
        protected override void Initialize()
        {
            ...
            _internalObSignal = NewEmptySignal();
        ...
        public override void Calculate(int index)
        {
            ...
            EvaluateSignal(_internalObSignal, signalClose, candleDir, ref condInternal);
            ...
            DrawSignals(index, condInternal, condSwing, condFvg);
        }
        ...
        {
            // InternalPivotLength is now user-configurable (was hardcoded to 5).
            var iLen = Math.Max(1, InternalPivotLength);
            ...
            ProcessStructureCrosses(index, iLen);
        ...
        /// <summary>
        /// Checks whether a level has been crossed by the current bar.
        /// The source price (Close vs High/Low) is determined by the supplied
        /// StructureSource parameter.
        /// </summary>
        private bool CrossedUp(int index, double level, StructureSource source)
        {
            return source == StructureSource.HighLow
                ? Bars.HighPrices[index] > level
                : Bars.ClosePrices[index] > level;
        }

        private bool CrossedDown(int index, double level, StructureSource source)
        {
            return source == StructureSource.HighLow
        ...
        private void ProcessStructureCrosses(int index, int iLen)
        {
            // ── Internal (iBOS / iCHoCH) ─────────────────────────────────────
            if (!double.IsNaN(_internalHighLevel) && !_internalHighCrossed
                && CrossedUp(index, _internalHighLevel, InternalStructureSourceInput))
            {
                _internalHighCrossed = true;
                EmitStructure(index, true, false, _internalHighLevel, true);
                if (ShowInternalOrderBlocksInput)
                    StoreOrderBlockFromPivot(_internalHighIndex, true, 1, index, index);
            }

            if (!double.IsNaN(_internalLowLevel) && !_internalLowCrossed
                && CrossedDown(index, _internalLowLevel, InternalStructureSourceInput))
            {
                _internalLowCrossed = true;
                EmitStructure(index, false, false, _internalLowLevel, true);
                if (ShowInternalOrderBlocksInput)
                    StoreOrderBlockFromPivot(_internalLowIndex, true, -1, index, index);
            }

            // ── Swing (BOS / CHoCH) ───────────────────────────────────────────
            if (!double.IsNaN(_lastSwingHigh) && !_swingHighCrossed
                && CrossedUp(index, _lastSwingHigh, SwingStructureSourceInput))
            {
                _swingHighCrossed = true;
                EmitStructure(index, true, true, _lastSwingHigh, false);
                if (ShowSwingOrderBlocksInput)
                    StoreOrderBlockFromPivot(_lastSwingHighIndex, false, 1, index, index);
            }

            if (!double.IsNaN(_lastSwingLow) && !_swingLowCrossed
                && CrossedDown(index, _lastSwingLow, SwingStructureSourceInput))
            {
                _swingLowCrossed = true;
                EmitStructure(index, false, true, _lastSwingLow, false);
                if (ShowSwingOrderBlocksInput)
                    StoreOrderBlockFromPivot(_lastSwingLowIndex, false, -1, index, index);
            }
        }
        ...
        private void EmitStructure(int index, bool bullishBreak, bool swing, double level, bool isInternal)
        {
            ...
            if (bullishBreak)
            {
                if (CrossedUp(index, level, isInternal ? InternalStructureSourceInput : SwingStructureSourceInput))
                {
            ...
            {
                if (CrossedDown(index, level, isInternal ? InternalStructureSourceInput : SwingStructureSourceInput))
                {
                    var choch = (isInternal ? _internalTrend : _swingTrend) > 0;
                    if (isInternal)
                    {
        ...
        /// <summary>
        /// Combined OB lifecycle + signal touch logic.
        ///
        /// OB touch → signal filters applied here:
        ///
        ///   Filter 1 – Same-bar block:
        ///     A long signal cannot be fired by a bullish candle that is ALSO the bar
        ///     that caused the structure break (StructureBreakIndex == index).
        ///     Symmetrically for short signals and bearish candles.
        ///
        ///   Filter 2 – Cooldown after structure break:
        ///     index must be > ob.StructureBreakIndex + MinBarsAfterStructureBreak.
        ///     When MinBarsAfterStructureBreak == 0 this has no effect.
        /// </summary>
            ...
            for (var i = list.Count - 1; i >= 0; i--)
            {
                ...
                if (!ob.SignalFired && ob.Index < index)
                {
                    if (bullish && Bars.LowPrices[index] <= ob.Top)
                    {
                        ...
                        // Filter 2: cooldown
                        var cooldownSatisfied = index > ob.StructureBreakIndex + MinBarsAfterStructureBreak;
                        ...
                        if (ob.Index + MinDist < index && !isSameBreakBar && cooldownSatisfied)
                        {
                            ...
                            SetObSignal(ob, NewSignal(index, ob.Top, true, ob.Bottom));
                        }
                    }
                    else if (!bullish && Bars.HighPrices[index] >= ob.Bottom)
                    {
                        ...
                        if (ob.Index + MinDist < index && !isSameBreakBar && cooldownSatisfied)
                        {
                            ...
                            SetObSignal(ob, NewSignal(index, ob.Bottom, false, ob.Top));
                        }
                    }
                }
                ...
                if (!ShowAllHistoricalObs && i >= keep) { DeleteObVisual(ob); continue; }
        ...
        /// <summary>
        /// Creates and stores an OrderBlock from the pivot bar that preceded the
        /// structure break.
        /// <param name="structureBreakIndex">
        ///   The bar index at which the BOS/CHoCH was confirmed. Stored on the OB
        ///   for the same-bar and cooldown filters.
        /// </param>
        /// </summary>
        private void StoreOrderBlockFromPivot(int pivotIndex, bool isInternal, int bias, int index, int structureBreakIndex)
        {
            ...
            var ob = new OrderBlock
            {
                ...
                StructureBreakIndex = structureBreakIndex   // ← new field
            };
        ...
        private void UpdateFvgs(int index)
        {
            ...
            if (index != _lastDetectedFvgIndex && (bull || bear))
            {
                ...
                if (bull)
                    _fvgRecords.Insert(0, new FvgRecord { Max = Bars.LowPrices[index], Min = Bars.HighPrices[index - 2], IsBull = true,  DetectionIndex = index, SignalFired = false });
                else
                    _fvgRecords.Insert(0, new FvgRecord { Max = Bars.LowPrices[index - 2], Min = Bars.HighPrices[index], IsBull = false, DetectionIndex = index, SignalFired = false });
            }
        }
        ...
        // ════════════════════════════════════════════════════════════════════════
        //  Signal Engine – FVG touch handler
        // ════════════════════════════════════════════════════════════════════════
        private void HandleFvgSignals(int index)
        {
            ...
            for (var i = _fvgRecords.Count - 1; i >= 0; i--)
            {
                ...
                if (r.IsBull)
                {
                    ...
                    if (cLow < r.Min) { _fvgRecords.RemoveAt(i); continue; }
                }
                else
                {
                    ...
                    if (cHigh > r.Max) { _fvgRecords.RemoveAt(i); continue; }
                }
            }
        }
        ...
        // ════════════════════════════════════════════════════════════════════════
        private void DrawSignals(int index, int condInternal, int condSwing, int condFvg)
        {
            var offset = SignalOffsetPips * Symbol.PipSize;
            var hasLongObSignal  = condInternal == 1 || condSwing == 1;
            var hasShortObSignal = condInternal == -1 || condSwing == -1;

            if (ShowSignalsOb && condInternal == 1)
            ...
            if (ShowSignalsOb && condSwing == 1)
                DrawSignalIcon($"swing_ob_buy_{index}_{_shapeId++}",  ChartIconType.UpArrow,   index, Bars.LowPrices[index],  SwingBullishColorInput,      -offset);
            if (ShowSignalsOb && condSwing == -1)
                DrawSignalIcon($"swing_ob_sell_{index}_{_shapeId++}", ChartIconType.DownArrow, index, Bars.HighPrices[index], SwingBearishColorInput,       offset);
            ...
            if (ShowSignalDots)
            {
                if (hasLongObSignal || condFvg == 1)  LongSignal[index]  = Bars.LowPrices[index]  - offset;
                if (hasShortObSignal || condFvg == -1) ShortSignal[index] = Bars.HighPrices[index] + offset;
            }

            if (condInternal == 1)
            {
                LongInternalObBottom[index] = _internalObSignal.ObSlLevel;
                LongObBottom[index]         = _internalObSignal.ObSlLevel;
            }
            if (condInternal == -1)
            {
                ShortInternalObTop[index] = _internalObSignal.ObSlLevel;
                ShortObTop[index]         = _internalObSignal.ObSlLevel;
            }

            if (condSwing == 1)
            {
                LongSwingObBottom[index] = _swingObSignal.ObSlLevel;
                if (double.IsNaN(LongObBottom[index]))
                    LongObBottom[index] = _swingObSignal.ObSlLevel;
            }
            if (condSwing == -1)
            {
                ShortSwingObTop[index] = _swingObSignal.ObSlLevel;
                if (double.IsNaN(ShortObTop[index]))
                    ShortObTop[index] = _swingObSignal.ObSlLevel;
            }
        ...
        private void SetObSignal(OrderBlock ob, SignalState signal)
        {
            if (ob.Internal)
                _internalObSignal = signal;
            else
                _swingObSignal = signal;
        }
        ...
        private void RemoveOld(string prefix, int keep) { /* stub – extend if needed in Present mode */ }
